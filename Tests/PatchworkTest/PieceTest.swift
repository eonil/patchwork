...
final class PieceTest: XCTestCase {
    func testLeafResolutions() {
        do {
            ...
            let p2 = p1.layout(in: CGRect(x: 10, y: 20, horizontal: 100, vertical: 200))
            XCTAssertEqual(p2.frame, CGRect(x: 10 + 50, y: 20 + 100, horizontal: 0, vertical: 0))
        }
        do {
            ...
            let p2 = p1.layout(in: CGRect(x: 0, y: 0, horizontal: 100, vertical: 100))
            XCTAssertEqual(p2.frame, CGRect(x: 45, y: 45, horizontal: 10, vertical: 10))
        }
        do {
            ...
            XCTAssertEqual(p1.content.fittingSize, .zero)
            let p2 = p1.layout(in: CGRect(x: 10, y: 20, horizontal: 100, vertical: 200))
            XCTAssertEqual(p2.frame, CGRect(x: 10, y: 20, horizontal: 100, vertical: 200))
        }
        do {
            let v = FixedSizedView(frame: CGRect(x: 0, y: 0, horizontal: 20, vertical: 40))
            let p = Piece(sizing: .fitContent, content: .view(v))
            let p1 = ResolvedPiece(from: p)
            XCTAssertEqual(p1.sizing, p.sizing)
            XCTAssertEqual(p1.content.fittingSize, CGSize(horizontal: 20, vertical: 40))
            let p2 = p1.layout(in: CGRect(x: 10, y: 20, horizontal: 100, vertical: 200))
            XCTAssertEqual(p2.frame, CGRect(x: 10+50-10, y: 20+100-20, horizontal: 20, vertical: 40))
        }
    }
    func testStitchResolution() {
        ...
        do {
            ...
            let p1 = ResolvedPiece(from: p)
            let p2 = p1.layout(in: CGRect(x: 0, y: 0, horizontal: 100, vertical: 200))
            XCTAssertEqual(p2.frame, CGRect(x: 50 - 5, y: 100 - 10, horizontal: 10, vertical: 20))
        }
        do {
            ...
            let p1 = ResolvedPiece(from: p)
            let p2 = p1.layout(in: CGRect(x: 0, y: 0, horizontal: 100, vertical: 200))
            XCTAssertEqual(p2.frame, CGRect(x: 0, y: 0, horizontal: 100, vertical: 200))
            guard let x2 = p2.content.stitchSublayouts else { return XCTFail() }
            guard x2.count == 2 else { return XCTFail() }
            XCTAssertEqual(x2[0].frame, CGRect(x: 0, y: 100 - 10, horizontal: 10, vertical: 20))
            XCTAssertEqual(x2[1].frame, CGRect(x: 10, y: 0, horizontal: 90, vertical: 200))
        }
        do {
            let p = Piece(sizing: .fillContainer, content: .stitch(Stitch(version: 1, content: {
                StitchContent(axis: .x, segments: [
            ...
            XCTAssertEqual(x3[0].frame, CGRect(x: 0, y: 0, horizontal: 90, vertical: 100))
            XCTAssertEqual(x3[1].frame, CGRect(x: 0, y: 100, horizontal: 90, vertical: 100))
        }
    }
    func testStack() {
        do {
            ...
            let p1 = ResolvedPiece(from: p)
            let p2 = p1.layout(in: CGRect(x: 0, y: 0, horizontal: 100, vertical: 100))
            XCTAssertEqual(p2.frame, CGRect(x: 50 - 30, y: 50 - 30, horizontal: 60, vertical: 60))
            guard let x2 = p2.content.stackSublayouts else { return XCTFail() }
            guard x2.count == 3 else { return XCTFail() }
            XCTAssertEqual(x2[0].frame, CGRect(x: 30 - 10, y: 30 - 10, horizontal: 20, vertical: 20))
            XCTAssertEqual(x2[1].frame, CGRect(x: 30 - 20, y: 30 - 20, horizontal: 40, vertical: 40))
            XCTAssertEqual(x2[2].frame, CGRect(x: 30 - 30, y: 30 - 30, horizontal: 60, vertical: 60))
        }
    }
}
